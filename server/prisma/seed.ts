// prisma/seed.ts
import { PrismaClient, Role, Gender, DayOfWeek, RelationType } from '@prisma/client';
...
async function main() {
  ...
  const hashedPassword = async (pwd: string) => bcrypt.hash(pwd, 10);
  ...
  const doctor2 = await prisma.doctor.create({
    data: {
      ...
      specialityId: specialities[1].id, // Dermatology
  ...
  const doctor4 = await prisma.doctor.create({
    data: {
      ...
      specialityId: specialities[4].id, // Pediatrics
  ...
  // Reviews require appointment - using existing one
